...
class SSHChannel(Channel):
    ...
    def __init__(self, node, user=None, **args):
        ...
        self._ssh_timeout = Conf.get(const.CSM_GLOBAL_INDEX, const.SSH_TIMEOUT,
                                     const.DEFAULT_SSH_TIMEOUT)
    ...
    def connect(self):
        ...
        try:
            self._ssh.connect(self._node, username=self._user, timeout=self._ssh_timeout,
                              allow_agent=self.allow_agent, key_filename=self.key_filename)
    ...
    def execute(self, command, **kwargs):
        ...
        try:
            ...
            Log.exception(e)
            raise CsmError(errno.EIO, str(e))
        except Exception as e:
            Log.exception(e)
...
class AmqpChannel(Channel):
    ...
    def __init__(self, **kwargs):
        ...
        self.node1 = Conf.get(const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.NODE1}")
        self.node2 = Conf.get(const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.NODE2}")
        self.hosts = Conf.get(const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.RMQ_HOSTS}")
        self.port = Conf.get(const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.PORT}")
        self.virtual_host = Conf.get(const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.VHOST}")
        self.username = Conf.get(const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.UNAME}")
        self.password = Conf.get(const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.PASS}")
        self.exchange_type = Conf.get(const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.EXCH_TYPE}")
        self.retry_counter = Conf.get(
            const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.RETRY_COUNT}")
        self.durable = Conf.get(const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.DURABLE}")
        self.exclusive = Conf.get(const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.EXCLUSIVE}")
        self._setExchangeandQueue()

    def _setExchangeandQueue(self):
        if not self.is_actuator:
            self.exchange = Conf.get(const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.EXCH}")
            self.exchange_queue = Conf.get(
                const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.EXCH_QUEUE}")
            self.routing_key = Conf.get(
                const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.ROUTING_KEY}")
        elif self.is_actuator:
            self.exchange = Conf.get(
                const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.ACT_REQ_EXCH}")
            self.exchange_queue = Conf.get(
                const.CSM_GLOBAL_INDEX, f"{const.CHANNEL}.{const.ACT_REQ_EXCH_QUEUE}")
            if self.is_node1:
                self.routing_key = Conf.get(
                    const.CSM_GLOBAL_INDEX,
    ...
    def _declare_exchange_and_queue(self):
        if(self._connection and self._channel):
            try:
                ...
                Log.error(f'Exchange: [{self.exchange}], type: [{self.exchange_type}] cannot be '
                          f'declared. Details: {e}')
            try:
                self._channel.queue_declare(queue=self.exchange_queue, exclusive=self.exclusive,
                                            durable=self.durable)
                self._channel.queue_bind(exchange=self.exchange, queue=self.exchange_queue,
                                         routing_key=self.routing_key)
                Log.info(f'Initialized Exchange: {self.exchange}, '
                         f'Queue: {self.exchange_queue}, routing_key: {self.routing_key}')
            except AMQPError as e:
                Log.error(f'CSM Fails to initialize the queue. Details: {e}')
                Log.exception(e)
                raise CsmError(-1, str(e))
...
class AmqpComm(Comm):
    ...
    def _alert_callback(self, ct, ch, method, properties, body):
        ...
        # TODO: add ct, ch, properties handling
        ...
        try:
            ...
            if self._inChannel.channel():
                self._inChannel.channel().basic_consume(
                    self._inChannel.exchange_queue, partial(self._alert_callback, consumer_tag),
            ...
            But for the 2nd case since we are closing the app we should not try to re-connect.
            """
...
class AmqpActuatorComm(Comm):
    def __init__(self):
        Comm.__init__(self)
        self._outChannel_node1 = AmqpChannel(is_actuator=True, is_node1=True)
        self._outChannel_node2 = AmqpChannel(is_actuator=True, is_node1=False)
    ...
    def disconnect(self):
        try:
            Log.debug("Disconnecting AMQPActuator RMQ communication")
